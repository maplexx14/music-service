...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-28 (commit bbedfd4). Confidence: 97%.
...
- `backend/app/routers/flow.py` — 15 bug fixes, last fix today (bug magnet); 65 commits/90d
- `backend/tests/test_flow.py` — 8 bug fixes, last fix today (bug magnet); 33 commits/90d
...
### Code health
Three co-equal signals: defect risk 5.28/10 avg, hotspot health 3.35/10 (stable), worst `backend/app/routers/soundcloud.py` at 1.0/10 · maintainability 7.16/10 · performance risk 83 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
