...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-05 (commit bad31a8). Confidence: 95%.
...
### Code health
Three co-equal signals: defect risk 5.54/10 avg, hotspot health 3.18/10 (stable), worst `backend/app/routers/soundcloud.py` at 1.0/10 · maintainability 7.08/10 · performance risk 42 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `backend/app/routers/ytdlp.py` — untested hotspot — impact −2.0
- `frontend/src/store/playerStore.js` — untested hotspot — impact −2.0
- `backend/app/routers/tracks.py` — change entropy — impact −1.4
- `frontend/src/store/playerStore.js` — change entropy — impact −1.4
- `backend/app/routers/ytdlp.py` — change entropy — impact −1.3
