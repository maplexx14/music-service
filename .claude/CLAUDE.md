...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-23 (commit 3f4ac51). Confidence: 97%.
...
- `backend/app/routers/flow.py` — 13 bug fixes, last fix today (bug magnet); 58 commits/90d
- `backend/tests/test_flow.py` — 8 bug fixes, last fix today (bug magnet); 30 commits/90d
...
### Code health
Three co-equal signals: defect risk 5.29/10 avg, hotspot health 3.5/10 (stable), worst `backend/app/routers/soundcloud.py` at 1.0/10 · maintainability 7.14/10 · performance risk 63 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
