...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-22 (commit a73ed83). Confidence: 97%.
...
- `backend/app/routers/flow.py` — 12 bug fixes, last fix today (bug magnet); 54 commits/90d
- `backend/tests/test_flow.py` — 7 bug fixes, last fix today (bug magnet); 27 commits/90d
...
### Code health
Three co-equal signals: defect risk 5.28/10 avg, hotspot health 3.46/10 (stable), worst `backend/app/routers/soundcloud.py` at 1.0/10 · maintainability 7.17/10 · performance risk 59 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
