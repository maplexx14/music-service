...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-21 (commit 0bc6b83). Confidence: 97%.
...
- `backend/app/routers/flow.py` — 11 bug fixes, last fix today (bug magnet); 52 commits/90d
- `backend/tests/test_flow.py` — 6 bug fixes, last fix today (bug magnet); 25 commits/90d
- `frontend/src/components/Player.jsx` — 5 bug fixes, last fix 7 days ago (bug magnet); 45 commits/90d
- `backend/app/routers/recommendations.py` — 4 bug fixes, last fix today (bug magnet); 22 commits/90d
- `backend/tests/test_recommendation_quality.py` — 3 bug fixes, last fix today; 4 commits/90d

### Code health
Three co-equal signals: defect risk 5.28/10 avg, hotspot health 3.44/10 (stable), worst `backend/app/routers/soundcloud.py` at 1.0/10 · maintainability 7.16/10 · performance risk 59 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `docker-compose.yml` — untested hotspot — impact −2.0
