...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-23 (commit 47675f6). Confidence: 97%.
...
- `backend/app/routers/flow.py` — 13 bug fixes, last fix yesterday (bug magnet); 60 commits/90d
- `backend/tests/test_flow.py` — 8 bug fixes, last fix yesterday (bug magnet); 31 commits/90d
- `frontend/src/components/Player.jsx` — 5 bug fixes, last fix 9 days ago (bug magnet); 45 commits/90d
- `backend/app/routers/recommendations.py` — 4 bug fixes, last fix yesterday (bug magnet); 24 commits/90d
- `backend/tests/test_recommendation_quality.py` — 3 bug fixes, last fix yesterday; 4 commits/90d

### Code health
Three co-equal signals: defect risk 5.31/10 avg, hotspot health 3.52/10 (stable), worst `backend/app/routers/soundcloud.py` at 1.0/10 · maintainability 7.13/10 · performance risk 73 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `backend/app/main.py` — untested hotspot — impact −2.0
