...
export function swapTo(url) {
  ...
  // Старый элемент не глушим здесь и вообще не глушим, пока новый не запоёт.
  // Между play() и реальным началом воспроизведения проходит до ~170 мс (лог с
  // устройства: play:call 55.484 → playing 55.651), и если в это окно замолчать,
  // не играет НИКТО. Аудиосессия при этом схлопывается: звук потом появляется,
  // но Now Playing остаётся привязанным к брошенному элементу — на экране
  // блокировки ▶ поверх звучащего трека и мёртвая шкала.
  //
  // Громкость убираем там, где это работает. На iOS не сработает: volume у
  // media-элемента доступен только для чтения, громкость там системная. Значит
  // на устройстве перекрытие СЛЫШНО — доли секунды двух треков сразу. Это
  ...
  // осталась подстраховкой на случай переиспользования элемента: play() с
  // конца мгновенно выстрелил бы ended и перещёлкнул трек дальше.
...
export function finishSwap(expectedActive = null) {
  ...
  try {
    ...
    // прогрев, и немой слот означал бы беззвучный трек через один.
    previous.volume = sharedVolume
  } catch {
    /* noop */
  }
  // Освобождаем ВСЕГДА, даже докачанный до конца. Раньше такой оставляли
  // заряженным ради мгновенной кнопки ◀ — и это ломало системный виджет:
  // WebKit выбирает «now playing» среди элементов с живым источником и
  // продолжал считать таковым оставленный, а он на паузе. На экране блокировки
  // это выглядело как ▶ поверх честно звучащего трека и мёртвая шкала
  // перемотки — шкала показывала длительность БРОШЕННОГО трека (в логе с
  // устройства: swap:finish dur=30 при играющем dur=133).
  //
  // Цена — «предыдущий трек» больше не мгновенный, он идёт обычным путём с
  // загрузкой. Правильное состояние виджета важнее: оно нужно на каждом треке,
  // а кнопка ◀ — изредка.
  release(previous)
...
// заряженный трек больше не следующий. Важно не «на будущее», а прямо сейчас —
// заряженный элемент может всё ещё тянуть байты, а на узком канале это отбирает
