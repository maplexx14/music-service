...
function Player() {
  ...
  const handleArtistClick = (event, artistName) => {
    event.stopPropagation()
    if (!artistName) return
    navigate(`/artist/${encodeURIComponent(artistName)}`, { state: { artistName } })
  }
  ...
  }, [isPlaying, currentTrack?.id, setCurrentTime])
  ...
  const playedTracksCounterRef = useRef(0)
  ...
  useEffect(() => {
    ...
    playedTracksCounterRef.current += 1

    api.post(`/tracks/${currentTrack.id}/play`).catch((error) => {
      console.error('Error recording track play:', error)
    })

    if (playedTracksCounterRef.current % 3 === 0) {
      window.dispatchEvent(new CustomEvent('recommendations:refresh', {
        detail: {
          reason: 'every_3_tracks',
          playedCount: playedTracksCounterRef.current,
        },
      }))
    }
  }, [currentTrack?.id])
  ...
  const renderTrackSlide = (track) => (
    <div className="player-slide" style={{ minWidth: '100%' }}>
      ...
      <div className="player-info">
        ...
        <button
          type="button"
          className="player-track-artist player-track-artist-link"
          onClick={(event) => handleArtistClick(event, track.artist)}
        >
          {track.artist}
        </button>
      </div>
