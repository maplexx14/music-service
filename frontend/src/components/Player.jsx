...
function PlayerInner() {
  ...
  useEffect(() => {
    ...
    // элемент реально выдаёт звук. Смена src останавливает элемент; если он к
    // этому моменту УЖЕ молчал — предыдущее переключение ещё качало трек, —
    // сессии нет, и новый фоновый старт система не разрешает. Виджет при этом
    // не «замирает», а пропадает совсем («нет аудио»), и вернуть звук можно
    // только открыв PWA. Ровно это ловится при двух скипах подряд: первый
    // уходит с играющего трека и проходит, второй — с ещё молчащего.
    //
    ...
    const canSwapSourceNow = () => {
      ...
      return !audio.paused && audio.currentTime > 0 && audio.readyState >= audio.HAVE_CURRENT_DATA
    }
    const handlers = {
      play: async () => {
        const audio = audioRef.current
