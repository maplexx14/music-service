...
function FullScreenPlayer() {
  ...
  const calcTimeFromEvent = useCallback((event) => {
  ...
  const handleSeekStart = useCallback((event) => {
    ...
    setIsDragging(true)
    setDragTime(calcTimeFromEvent(event))
  }, [calcTimeFromEvent])
  ...
  useEffect(() => {
    ...
    const handleEnd = (event) => {
      const finalTime = calcTimeFromEvent(event)
  ...
  const handleBarClick = (event) => {
    if (isDragging) return
    const newTime = calcTimeFromEvent(event)
    const audio = getAudioElement()
    if (audio) {
