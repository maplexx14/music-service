...
# куки не сортирует. В значениях могут быть '$': Docker Compose трактует
# ``$NAME`` как подстановку переменной и молча вырезает неизвестный ``NAME``.
# Удвоение '$' — документированный способ передать литерал через Compose;
# контейнер получит исходный одиночный символ. Без этого даже команда
# ``docker compose exec`` портила бы cookie-сессию и PO-token не выпускался.
# Переводов строки в Netscape-значении быть не может (формат табличный).
umask 077
tmp="$(mktemp)"
