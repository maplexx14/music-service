...
def test_single_play_is_heard_but_not_taste(db):
    """Одно прослушивание закрывает НОВИЗНУ, не создавая вкуса.

    Порог _PLAYED_ARTIST_MIN_PLAYS прав, когда решается, считать ли артиста
    вкусом, но «слышал один раз» уже не «новое имя»: до разделения понятий
    такие артисты занимали слоты разведки при выкрученном ползунке, хотя
    пользователь их уже слушал.
    """
    user = create_user(db, username="heard-not-taste-user")
    played = Track(
        title="случайный трек",
        artist="OnceHeardArtist",
        duration=100,
        source="local",
        file_path="minio://music/once-heard.mp3",
    )
    db.add(played)
    db.commit()
    db.execute(
    ...
    profile = _taste_profile(db, user.id)
    assert "onceheardartist" not in profile["artist_weight"], profile["artist_weight"]
    assert "onceheardartist" in profile["heard_artist_keys"], profile[
        "heard_artist_keys"
    ]


def test_wave_play_event_is_heard_without_materialization(db):
    """Прослушанный в волне внешний артист — слышанный, даже без импорта.

    POST /tracks/{id}/play требует локальный track_id, поэтому единственный
    след незаматериализованного прослушивания — событие телеметрии. Без этого
    артист, услышанный прямо в волне, навсегда оставался «новым», и каждая
    подгрузка снова тратила на него квоту разведки.
    """
    user = create_user(db, username="heard-from-wave-user")
    db.execute(
        recommendation_events.insert().values(
            user_id=user.id,
            source="ytmusic",
            external_id="wave-1",
            artist="WaveOnlyArtist",
            title="трек из волны",
            event_type="play",
            surface="flow",
    ...
    profile = _taste_profile(db, user.id)
    assert "waveonlyartist" in profile["heard_artist_keys"], profile[
        "heard_artist_keys"
    ]
    assert "waveonlyartist" not in profile["artist_weight"], profile["artist_weight"]
