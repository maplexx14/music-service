...
from app.schemas import TrackResponse, TrackCreate, RecentTrackResponse
...
@router.get("/me/recent", response_model=List[RecentTrackResponse])
async def get_recently_played_tracks(
    ...
    """Get recently played tracks for the current user ordered by last played time."""
    from app.models import user_track_plays
    ...
    stmt = (
        select(Track, user_track_plays.c.last_played)
        .join(user_track_plays, Track.id == user_track_plays.c.track_id)
        .where(
            (user_track_plays.c.user_id == current_user.id) &
            (user_track_plays.c.last_played >= since_dt)
        )
        .order_by(user_track_plays.c.last_played.desc())
        .offset(skip)
        .limit(limit)
    )
    result = await db.execute(stmt)
...
@router.delete("/me/recent", status_code=status.HTTP_200_OK)
async def clear_recently_played(
    ...
    await db.execute(
        delete(user_track_plays).where(user_track_plays.c.user_id == current_user.id)
    )
    await db.commit()
