...
# артистах выполнимо единственным способом — ротацией, поэтому поднимать его
# бессмысленно, если порцию набирают три-четыре имени. За это отвечают
# _MIN_BATCH_ARTISTS ниже (состав порции) и стохастический выбор в
# diversity.interleave_artists (порядок).
_MIN_ARTIST_GAP = 4
# Сколько РАЗНЫХ артистов должно набирать порцию. Кап на артиста выводится из
# этого числа, а не задаётся отдельным: при limit=15 прежний кап (6) позволял
# трём именам занять всю порцию, а три имени по 15 позициям раскладываются
# только ротацией — «первый артист, второй, третий, снова первый». Разнос с
# этим сделать уже ничего не может: он меняет порядок, но не состав.
_MIN_BATCH_ARTISTS = 5
...
async def get_flow(
    ...
    # Кап выведен из числа имён, которыми порция должна набираться: при limit=15
    # это 3 трека на артиста, то есть минимум пять разных исполнителей. Прежние
    # max(4, limit // 2) давали 6 — сорок процентов порции одному имени.
    artist_cap = min(_MAX_PER_ARTIST, max(2, math.ceil(limit / _MIN_BATCH_ARTISTS)))
    ...
    # порция из трёх имён, которую разнос способен выдать только ротацией.
