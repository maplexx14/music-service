...
class ArtistPageResponse(BaseModel):
    ...
    Порядок на фронте — по популярности (у треков external это метрика
    провайдера: views у YouTube Music, playback_count у SoundCloud), поэтому
    play_count у внешних треков обязана приезжать заполненной, когда
    провайдер её отдаёт.
